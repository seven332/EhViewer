...
public class ViewDragHelper {
    ...
    // TODO make it large enough
    private static final int EDGE_SIZE_INTERCEPT = 20;
    private static final int EDGE_SIZE = 400; // dp
    ...
    private int mEdgeSizeIntercept;
    ...
    private ViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        ...
        mEdgeSizeIntercept = (int) (EDGE_SIZE_INTERCEPT * density + 0.5f);
    ...
    private void saveInitialMotionIntercept(float x, float y, int pointerId) {
        ensureMotionHistorySizeForId(pointerId);
        mInitialMotionX[pointerId] = mLastMotionX[pointerId] = x;
        mInitialMotionY[pointerId] = mLastMotionY[pointerId] = y;
        mInitialEdgesTouched[pointerId] = getEdgesTouchedIntercept((int) x, (int) y);
    }
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                saveInitialMotionIntercept(x, y, pointerId);
    ...
    public void processTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if ((mPointersDown & (1 << MotionEventCompat.getPointerId(ev, 0))) == 0) {
                    final float x = ev.getX();
                    final float y = ev.getY();
                    saveInitialMotion(x, y, MotionEventCompat.getPointerId(ev, 0));
                }
    ...
    private int getEdgesTouched(int x, int y) {
        int result = 0;
        int centerX = mParentView.getLeft() + mParentView.getRight()/2;
        int centerY = mParentView.getLeft() + mParentView.getRight()/2;
        if (x < centerX) result |= EDGE_LEFT;
        if (y < centerY) result |= EDGE_TOP;
        if (x > centerX) result |= EDGE_RIGHT;
        if (y > centerY) result |= EDGE_BOTTOM;

        return result;
    }
    
    private int getEdgesTouchedIntercept(int x, int y) {
        int result = 0;

        if (x < mParentView.getLeft() + mEdgeSizeIntercept) result |= EDGE_LEFT;
        if (y < mParentView.getTop() + mEdgeSizeIntercept) result |= EDGE_TOP;
        if (x > mParentView.getRight() - mEdgeSizeIntercept) result |= EDGE_RIGHT;
        if (y > mParentView.getBottom() - mEdgeSizeIntercept) result |= EDGE_BOTTOM;
