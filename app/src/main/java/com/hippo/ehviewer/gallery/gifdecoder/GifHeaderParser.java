...
public class GifHeaderParser {
    ...
    private byte[] mTemp = new byte[2];

    public GifHeaderParser setData(UniRandomReadFile data) {
        reset();
        rawData = data;
        return this;
    }
    ...
    public GifHeader parseHeader() {
        ...
        try {
            ...
            e.printStackTrace();
            header.status = GifDecoder.STATUS_READ_ERROR;
        }
    ...
    private void skip() throws IOException {
        ...
        do {
            blockSize = read();
            rawData.seek(rawData.position() + blockSize);
        } while (blockSize > 0);
    }
    ...
    private int readShort() throws IOException {
        // Read 16-bit value.
        try {
            rawData.read(mTemp, 0, 2);
            return (mTemp[1] << 8 & 0xff00) | (mTemp[0] & 0xff);
        } catch (IOException e) {
            header.status = GifDecoder.STATUS_READ_ERROR;
            throw e;
        }
